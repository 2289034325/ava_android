...
public class WritingServiceImp implements WritingService {
  ...
  @Inject
  WritingServiceImp(ServiceUtil serviceUtil) {
  ...
  @Override
  public Observable<List<WritingArticle>> getArticleList() {
    return Observable.create(emitter -> {
      if (serviceUtil.isThereInternetConnection()) {
        try {
          Map<String,String> tokenHeader = serviceUtil.getTokenHeader();
          String responseString =  ApiConnection.create(Method.GET,ServiceConsts.API_WRITING_LIST,null,tokenHeader).call();

          if (responseString != null) {
            final Type responseType = new TypeToken<List<WritingArticle>>() {}.getType();
            List<WritingArticle> bms = this.gson.fromJson(responseString, responseType);
            emitter.onNext(bms);
            emitter.onComplete();
          } else {
